...
def create_grads_for_each_client(grad_fn):
  ...
  def client_step(client_step_state, batch):
    ...
    grads_sum = exp_tree_util.tree_add(
...
def mime(
  ...
  Returns:
    FederatedAlgorithm
  """
  grad_fn = model.grad(per_example_loss, regularizer)
  ...
  def apply(
    ...
    grads_sum_total, num_sum_total = exp_tree_util.tree_sum(
        (co for _, co in grads_for_each_client(server_state.params,
                                               grads_batch_clients)))
    server_grads = exp_tree_util.tree_inverse_weight(grads_sum_total,
                                                     num_sum_total)
