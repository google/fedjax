...
def hyp_cluster(
  ...
  evaluator = model.AverageLossEvaluator(per_example_loss, regularizer)
  trainer = ClientDeltaTrainer(
      model.grad(per_example_loss, regularizer), client_optimizer)
...
def maximization_step(
    evaluator: model.AverageLossEvaluator, cluster_params: List[Params],
    clients: Sequence[Tuple[federated_data.ClientId,
                            client_datasets.ClientDataset, PRNGKey]],
...
def expectation_step(trainer: ClientDeltaTrainer, cluster_params: List[Params],
  ...
  for client_id, delta_params in trainer.train_per_client_params([
    ...
    cluster_delta_params_sum[cluster_id] = exp_tree_util.tree_add(
  ...
  for delta_params_sum, num_examples_sum in zip(cluster_delta_params_sum,
    ...
    if num_examples_sum > 0:
      cluster_delta_params.append(
          exp_tree_util.tree_inverse_weight(delta_params_sum, num_examples_sum))
    else:
...
class ModelKMeansInitializer:
  ...
  def __init__(self,
    ...
    self._trainer = ClientParamsTrainer(
        model.model_grad(model_, regularizer), client_optimizer)
    self._evaluator = model.AverageLossEvaluator(
        model.model_per_example_loss(model_), regularizer)
...
def kmeans_init(num_clusters: int, init_params: Params,
                ...
                train_batch_hparams: client_datasets.ShuffleRepeatBatchHParams,
                evaluator: model.AverageLossEvaluator,
...
class HypClusterEvaluator:
  ...
  def __init__(self,
    ...
    Args:
      ...
      regularizer: Optional regularizer.
    """
    self._maximization_step_evaluator = model.AverageLossEvaluator(
        model.model_per_example_loss(model_), regularizer)
    self._model_evaluator = model.ModelEvaluator(model_)
